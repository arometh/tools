 ...
 */
public class OdooRecordSet {

	// use java logging
	private static Logger jLog = Logger.getLogger(OdooRecordSet.class
			.getName());

	private Vector<OdooRecord> records;
	 ...
	 */
	public OdooRecordSet(Object[] readresult) {
		records = new Vector<OdooRecord>();
		if (readresult != null) {
			for (int i = 0; i < readresult.length; i++) {
				records.add(new OdooRecord(readresult[i]));
			}
	 ...
	 * @return
	 */
	public boolean add(OdooRecord record) {
		if (record != null) {
	 ...
	 */
	public boolean addAll(OdooRecordSet c) {
		boolean allOK = false;
		Iterator<OdooRecord> i = c.iterator();
		while (i.hasNext()) {
			OdooRecord o = i.next();
	...
	/**
	 * Creates and returns a copy of this object. Subsequent object will be copies, too. 
	 * @return copy of this record set 
	 */
	public OdooRecordSet copy() {
		OdooRecordSet c = new OdooRecordSet();
		
		Iterator<OdooRecord> i = this.iterator();
		while (i.hasNext()) {
			OdooRecord r = i.next();
			OdooRecord rc = r.copy();
			c.add(rc);
		}
		return c;
	}
	 ...
	 * @return identified Record, null if none found
	 */
	public OdooRecord get(int id) {
		Iterator<OdooRecord> i = records.iterator();
		while (i.hasNext()) {
			OdooRecord n = i.next();
	 ...
	 */
	public OdooRecord get(String key, Object value) {
		Iterator<OdooRecord> i = records.iterator();
		while (i.hasNext()) {
			OdooRecord n = i.next();
	 ...
	 */
	public Iterator<OdooRecord> iterator() {
	 ...
	 * 
	 */
	public void relate(String relation, OdooRecordSet insert) {
			...
			if (r.containsKey(relation)) {
				...
				if (!(relation_id_obj == null)) {
					...
					try {
						...
						OdooRecordSet subset = insert.getIDSubset(oList);
					...
					if (!isList) {
						...
						// inserted
						OdooRecord related_object = insert.get(relation_id);
	 ...
	 */
	public OdooRecordSet getIDSubset(Object[] lst) {
		// bail out
		if (lst == null)
			return null;
		if (lst.length <= 0)
		...
		OdooRecordSet result = new OdooRecordSet(null);
	 ...
	 */
	public OdooRecordSet subset(String key, Object value) {
		OdooRecordSet r = new OdooRecordSet();
	 ...
	 */
	public OdooRecordSet subset(HashMap<String, Object> keyvalues) {
		OdooRecordSet r = new OdooRecordSet();
	 ...
	 */
	public OdooRecord search(String fieldname, Object search) {
	...
	public boolean contains(String fieldname, Object search) {
		OdooRecord r = this.search(fieldname, search);
		return (r != null);
	}
	 ...
	 */
	public boolean contains(OdooRecord o) {
	 ...
	 */
	public boolean containsAll(OdooRecordSet c) {
		boolean allOK = true;
		Iterator<OdooRecord> i = c.iterator();
		while (i.hasNext()) {
			OdooRecord o = i.next();
	 ...
	 */
	public void clear() {
		this.records = new Vector<OdooRecord>();
	}
	 ...
	 */
	public boolean remove(OdooRecord o) {
	 ...
	 */
	public boolean removeAll(OdooRecordSet c) {
		boolean allOK = false;
		Iterator<OdooRecord> i = c.iterator();
		while (i.hasNext()) {
			OdooRecord o = i.next();
	 ...
	 */
	public boolean retainAll(OdooRecordSet c) {
		if (c == null) throw new NullPointerException();
		
		OdooRecordSet to_remove = new OdooRecordSet();
		
		Iterator<OdooRecord> i = this.records.iterator();
		while (i.hasNext()) {
			OdooRecord o = i.next();
